...
/** HTTP adapter for the App management port; it never exposes Runtime as a resource. */
@RestController
@RequestMapping("/api/v1/app")
class AppController {
    private final KuudraApp app;
    private final KuudraAppHttpApplication.DaemonTerminator terminator;
    AppController(KuudraApp app, KuudraAppHttpApplication.DaemonTerminator terminator) { this.app = app; this.terminator = terminator; }
    ...
    @PostMapping("/terminate") Map<String, Object> terminate() { terminator.terminate(); return Map.of("accepted", true); }
    ...
    private static void send(SseEmitter emitter, SystemEvent event) {
