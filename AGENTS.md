...
- `kuudra-web` HTTP adapters are split by resource domain under the `controller` package, with one `/api/v1/{resource-domain}` base path per Controller. Plugin-provided `Component` views are templates exposed by `component-templates`; manifest-declared `ComponentResource` views are instances exposed by `components`. EventSource instances use the generic Component API rather than a duplicate HTTP controller. Controllers depend only on `KuudraApp` and must not collapse multiple resource domains into one catch-all Controller.
...
- Kuudra Web OpenAPI provides an aggregate default `all` group and resource-domain `app`, `flows`, `resource-documentation`, `components`, `component-templates`, `sessions`, `system-events`, and `plugins` groups. `/components` exposes every manifest Component instance and its actual state/imports/capabilities. Keep Chinese class-level tags and operation summaries synchronized when endpoints change; grouping must not change REST paths or expose Runtime.
...
- Every enabled periodic reconciliation cycle emits TRACE `reconciliation.cycle.started/completed`; actual component observed-state transitions emit DEBUG `component.state.changed`. Preserve these levels and structured fields when changing reconciliation.
