...
- Global YAML contains root `home-directory`, runtime queue/worker settings, logging level/output switches and `global-context`. All schema keys use lowercase kebab-case. The packaged default sets `home-directory: .kuudra`, `logging.level: info`, and enables console/file output. App initialization always ensures the home, fixed `plugins/`, fixed `flows/`, and fixed `logs/` directories exist; if home `config.yaml` is absent, it copies the packaged `config.yaml` with create-only semantics and never overwrites an existing file. Deleting a broken home config and restarting restores packaged defaults.
...
- Cross-Flow component reuse must not be inferred from an equal Java type or plugin component reference. The planned model separates plugin definitions, explicitly named App-scoped instances, and per-Flow bindings. Plugins declare whether an implementation is shareable/thread-safe; deployment config explicitly chooses a shared instance. EventSource instances may then fan out to multiple Flow targets and are started/stopped once by App. Flow-scoped instances remain the default until that model is implemented.
- The accepted target configuration model is K8s-style resource manifests using lowercase kebab-case keys (`api-version`, `kind`, `metadata`, `spec`, `status`). Component resources declare named instances and options; Flow resources use `imports` plus `edges`. The target home directory is `<home>/manifests/` with recursive YAML discovery, replacing the top-level `flows/` only after a staged migration. App will reconcile desired resources and expose generic resource APIs; current Flow YAML and APIs remain authoritative until implementation lands. See `docs/kuudra-resource-manifests.md` before implementing any part of this migration.
