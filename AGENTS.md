...
- Controller handlers execute asynchronously. A named `@EventHandler` method returns `CompletionStage<Void>` and uses `EventHandlerContext` for arguments, execution control, emission and current-Session control. Runtime work leases, not business events, determine Session completion. Synchronous Adapter/Interpreter/Ingress/Egress poll without blocking, while long-running handlers may park at `checkpoint()` without releasing their Session lease.
