...
- `kuudra-web` HTTP adapters are split by domain under the `controller` package: App lifecycle, Flow, EventSource, Component resources, Session, plugins/components and SystemEvent streaming each have an independent Controller. Controllers depend only on `KuudraApp`; keep existing `/api/v1/app` paths stable and do not collapse the domains back into one catch-all Controller.
