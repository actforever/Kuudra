...
- `kuudra-web` HTTP adapters are split under the `controller` package. `/api/v1/runtime` groups Flow, manifest Component instances and Session; `/api/v1/plugin` groups Plugin and plugin-provided Component templates. EventSource instances use the generic Runtime Component API rather than a duplicate HTTP controller. The Runtime URL domain is organizational only: every Controller depends on `KuudraApp` and must not expose a Runtime object.
...
- Kuudra Web OpenAPI provides an aggregate default `all` group and domain `app`, `runtime`, `resource-documentation`, `system-events`, and `plugin` groups. `/runtime/components` exposes every manifest Component instance and its actual state/imports/capabilities. Keep Chinese class-level tags and operation summaries synchronized when endpoints change; grouping must not change REST paths or expose a Runtime object.
...
- Plugin archive scan `plugin.scan.started/completed` and successful startup stages `plugin.initialized/starting/active` are DEBUG diagnostics; keep them out of the default INFO startup log. Failure and shutdown visibility remains unchanged.
