...
- Flow is an immutable routing declaration and has no lifecycle state. App and individual Sessions support non-destructive pause/resume; pause preserves component instances, component state, contexts and queued events, and gates subsequent Event execution at Runtime safe points.
...
- `<home-directory>/state/kuudra.db` is the embedded SQLite StateStore. Startup transactionally imports manifests as the authoritative desired set, increments changed generations, deletes removed declarations, reconciles the persisted representation, then updates `observedGeneration` and phase. It stores resource control state only—not Sessions, event payloads, or plugin-owned data.
