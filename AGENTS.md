...
- Web Context closure (including Ctrl-C) must publish `web.shutdown.requested` before Spring destroys the App bean. Graceful shutdown must retain stage-level App/Runtime SystemEvents. Session drain waits are bounded by `runtime.shutdown-session-drain-timeout-ms`; plugin/component lifecycle stages currently rely on their returned CompletionStages and must log their start before joining.
- SystemEvent producers must not embed human-language log sentences. Treat the stable event `type` as a future I18n message key and structured `data` fields as template arguments; current console/file rendering remains English-key based.
- `kuudra-i18n` packages the default English `i18n/en.json`; App exposes an external-first `MessageResolver` and falls back to that catalog when an external resolver misses. Keep every core SystemEvent renderable through catalog placeholders. Future plugin catalogs must be identity-scoped (`plugin.<namespace>.<pluginId>.*`) rather than allowing plugins to override kernel keys; plugin-authored `plugin.log` free text remains untranslated until a separate keyed logger API is introduced.
