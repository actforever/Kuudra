...
`plugins/` is intentionally excluded by the root `.gitignore`. It is a local Maven aggregator for plugin implementations. The sibling `kuudra-default-plugin` is the ordinary deployable plugin `kuudra-official/default`; it is no longer a core Reactor module or implicitly registered by App. The external `kuudra-plugin-demos` workspace contains demonstration plugins and examples. Plugin builds expect `kuudra-api` and `kuudra-plugin` artifacts in the local Maven repository.
