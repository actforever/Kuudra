...
- A manifest file may contain multiple YAML documents separated by `---`; duplicate identities are still rejected across every file and document. `desiredState` is currently validated and converged once during App startup, not continuously reconciled: EventSource supports `running/stopped`, passive components support `active/inactive` (inactive resources cannot be imported), and Flow supports `active/paused/stopped`.
- `<home-directory>/state` is currently only created as a reserved directory and has no reader, writer, SQLite database, ResourceRepository, or reconciliation loop. Do not describe it as an implemented etcd equivalent. The intended future design is an embedded SQLite StateStore for persisted desired resources, generations and observed reconciliation status; manifests remain bootstrap/apply input rather than a second authoritative database.
