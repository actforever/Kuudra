...
- Kuudra Web OpenAPI provides an aggregate default `all` group and domain `app`, `runtime`, `plugin`, `resource-documentation`, and `system-events` groups. Knife4j uses ordered Chinese display names while the stable group identifiers continue to back `/v3/api-docs/{group}`. Controller-level OpenAPI tags use consistent English resource names; operation summaries remain Chinese. The `runtime` group merges Flow, manifest Component instances, and Sessions; the `plugin` group merges loaded plugins and Component templates. `/runtime/components` exposes every manifest Component instance and its actual state/imports/capabilities. Keep tags and operation summaries synchronized when endpoints change; grouping must not change REST paths or expose a Runtime object.
