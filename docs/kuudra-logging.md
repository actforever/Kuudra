...
1. 创建或截断 `logs/latest.log`；
2. 内核运行期间同步追加纯文本事件日志；
3. 正常停止时先取消总线订阅并停止文件 appender，保证缓冲已刷新；
4. 将 `latest.log` gzip 为 `yyyy-MM-dd-N.log.gz`，其中 `N` 从 1 开始选择当天第一个未占用序号；
5. 压缩成功后删除 `latest.log`。

若压缩或删除失败，停止过程会明确失败并保留可诊断文件。进程被强制终止时无法执行关闭归档，遗留的 `latest.log` 会在下一次启动按约定清空；需要保留崩溃日志时，应在重启前先复制该文件。

## 示例

```text
.kuudra/logs/
  latest.log                 # 仅在内核运行期间存在
  2026-08-22-1.log.gz
  2026-08-22-2.log.gz
```
