...
组件可以用 `@ComponentDoc` 提供用途、使用示例和生命周期阶段，用 `configuration = {@SpecProperty(...)}` 描述 `spec.options` 实例配置规约，用 `@EventEmission` 列出可能产生的业务事件。配置规约通过 Component 模板详情的 `documentation.configuration` 返回，包含 path、type、required、defaultValue、allowedValues、description 和 examples；examples 是原生 JSON 值列表，可以包含对象与数组。内核扫描后提供结构化查询：`GET /api/v1/plugins` 查询已加载插件，`GET /api/v1/component-templates` 查询全部 Component 模板，`GET /api/v1/component-templates/{type}/{namespace}/{name}` 查询模板详情。Knife4j 默认打开包含全部端点的 `all` 分组，也可切换到独立的 `plugins` 或 `component-templates` 分组。
