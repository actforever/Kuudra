...
组件可以用 `@ComponentDoc` 提供用途、使用示例和生命周期阶段，用 `@EventEmission` 列出可能产生的业务事件。内核扫描后提供结构化查询：`GET /api/v1/app/plugins` 查询已加载插件，`GET /api/v1/app/plugins/{namespace}/{pluginId}/components` 查询插件组件，`GET /api/v1/app/components/{type}/{namespace}/{name}` 查询组件详情。Knife4j 默认打开包含全部端点的 `all` 分组，也可切换到独立的 `plugins` 分组。
