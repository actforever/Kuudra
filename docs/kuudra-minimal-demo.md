...
当前 Demo 验证的是不依赖 JNativeHook 或 AWT Robot 的最小内核闭环；它使用受限 YAML 子集装配 Demo：
...
- `kuudra-plugin`：提供已加载插件的依赖排序、私有家目录和 `initialize → start → stop → destroy` 生命周期管理；Fat JAR 发现与隔离 ClassLoader 仍是后续迭代项。
