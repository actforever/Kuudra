...
内核版本的唯一维护入口是根目录 `.mvn/maven.config` 中的 `revision`。根项目和所有模块父坐标使用 Maven CI-friendly `${revision}`，flatten 插件在安装或发布时生成已解析的消费端 POM。`kuudra-app` 构建还会把同一 Maven 版本过滤到 `META-INF/kuudra/version.properties`；App 启动日志打印该版本，`KuudraApp.version()` 提供框架无关查询。不要在 Java 代码或其他模块 POM 中复制版本常量。
