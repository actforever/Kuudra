...
内置配置声明 `home-directory: .kuudra`，因此打包部署时用户配置默认为 `<jar-directory>/.kuudra/config.yaml`，插件 JAR 固定放在 `<jar-directory>/.kuudra/plugins/`，Flow YAML 固定放在 `<jar-directory>/.kuudra/flows/`。

首次启动时，Web 会通过 App 自动创建 `.kuudra/`、`.kuudra/plugins/` 和 `.kuudra/flows/`，并在缺少 `.kuudra/config.yaml` 时复制包内默认配置。现有配置不会被覆盖；配置损坏时可删除该文件并重启以恢复默认值。
