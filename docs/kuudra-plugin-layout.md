...
插件及其 Component 模板由 App 提供只读快照，并通过 Web 的 `/api/v1/plugin`、`/api/v1/plugin/{namespace}/{pluginId}` 和 `/api/v1/plugin/component-templates` 资源域公开。插件始终以 `namespace/pluginId` 隔离，同 ID、不同 namespace 的插件可以同时加载。Component 模板是否真正具有生命周期还会根据其接口实现自动识别；模板详情的 `documentation.configuration` 会原样返回结构化实例规约。
